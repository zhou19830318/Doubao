...
{
    lv_display_t *disp = board_get_lvgl_disp();
    ...
    lv_obj_remove_flag(s_detail_panel, LV_OBJ_FLAG_SCROLLABLE);
...
static void task_item_cb(lv_event_t *e)
{
    /* TODO(Task 8): 由 doubao 链路替换 — openclaw_get_info/openclaw_cron_toggle deleted in Task 1.
     * Previously toggled the tapped OpenClaw cron job and showed its detail panel. */
    (void)e;
}
...
void ui_tasks_refresh(void)
{
    ...
    // TODO(Task 8): 由 doubao 链路替换 — was `info->task_count == 0` (openclaw_info_t deleted in Task 1)
    if (true) {
    ...
    // TODO(Task 8): 由 doubao 链路替换 — openclaw task rows deleted in Task 1
    // } else {
