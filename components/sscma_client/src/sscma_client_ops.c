...
esp_err_t sscma_client_new(const sscma_client_io_handle_t io, const sscma_client_config_t *config, sscma_client_handle_t *ret_client)
{
    ...
    if (config->reset_gpio_num >= 0)
    {
        if (config->flags.reset_use_expander)
        {
            ...
            ESP_GOTO_ON_ERROR(esp_io_expander_set_dir(client->io_expander, (1ULL << config->reset_gpio_num), IO_EXPANDER_OUTPUT), err, TAG, "set GPIO direction failed");
        }
        else
        {
...
esp_err_t sscma_client_del(sscma_client_handle_t client)
{
    if (client)
    {
        if (client->reset_gpio_num >= 0)
        {
            ...
            {
                esp_io_expander_set_dir(client->io_expander, (1ULL << client->reset_gpio_num), IO_EXPANDER_OUTPUT);
                esp_io_expander_set_level(client->io_expander, (1ULL << client->reset_gpio_num), client->reset_level);
                esp_io_expander_set_dir(client->io_expander, (1ULL << client->reset_gpio_num), IO_EXPANDER_INPUT);
            }
            else
            {
                gpio_config_t io_conf = {
