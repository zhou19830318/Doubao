...
esp_err_t esp_websocket_client_destroy(esp_websocket_client_handle_t client)
{
    ...
    /* LOCAL PATCH (AIWatch): always join the ws task before freeing.
     * Upstream: destroy() skipped stop() when run==false — the abort path
     * (transport error) sets run=false + selected_for_destroying while the
     * task is STILL finishing its exit sequence, and the immediate free
     * raced the task's final accesses (freed-lock crash in
     * esp_websocket_client_task). The task never frees itself (see the
     * exit-path patch below), so this is the single free site. */
    if (client->run) {
        esp_websocket_client_stop(client);
    } else if (client->task_handle != NULL) {
        xEventGroupWaitBits(client->status_bits, STOPPED_BIT, false, true,
                            portMAX_DELAY);
...
static esp_err_t esp_websocket_client_recv(esp_websocket_client_handle_t client)
{
    ...
    do {
        /* Use a shorter timeout for recv (500ms instead of network_timeout_ms
         * which can be 10s). poll_read() already confirmed data is available,
         * so this only waits for the TLS record to arrive. Holding the lock
         * for 10s blocks every send_text() call from the dbws task — field
         * logs show hundreds of "Could not lock ws-client within 50 timeout"
         * errors per conversation turn. 500ms is a good balance: short enough
         * to keep lock contention manageable (send_text 200ms timeout succeeds
         * between recv fragments), but long enough for TLS records to arrive
         * even on slower networks. */
        int recv_timeout_ms = 500;
        rlen = esp_transport_read(client->transport, client->rx_buffer, client->buffer_size, recv_timeout_ms);
        if (rlen < 0) {
...
static void esp_websocket_client_task(void *pv)
{
    ...
    while (client->run) {
        ...
        case WEBSOCKET_STATE_CONNECTED:
            ...
            /* Note: lock must NOT be released during recv. Upstream holds
             * it across the whole recv for this reason — releasing it
             * creates a race: recv fails → abort_connection closes the
             * transport (frees TLS context) while the caller's send_text
             * is mid ssl_write → LoadProhibited / tlsf_free double-free.
             * The dbws drain path has a 50ms send timeout that correctly
             * handles lock contention ("Could not lock ws-client") without
             * crashing. */
            esp_err_t recv_ret = esp_websocket_client_recv(client);

            if (recv_ret == ESP_FAIL) {
    ...
    esp_transport_close(client->transport);
    client->state = WEBSOCKET_STATE_UNKNOW;
    /* LOCAL PATCH (AIWatch): STOPPED_BIT is the task's LAST access to
     * client memory, and the task NEVER frees itself — the caller's
     * destroy() joins on this bit and is the single free site. (Upstream
     * freed here under selected_for_destroying, which double-freed when
     * destroy() raced the teardown.) */
...
int esp_websocket_client_send_with_opcode(esp_websocket_client_handle_t client, ws_transport_opcodes_t opcode, const uint8_t *data, int len, TickType_t timeout)
{
    ...
    if (xSemaphoreTakeRecursive(client->lock, timeout) != pdPASS) {
        /* Rate-limited: during normal operation the ws task holds the lock
         * across esp_websocket_client_recv() (up to 500ms per fragment).
         * The dbws task's send_text() times out trying to acquire it — this
         * is EXPECTED contention, not an error. Log at WARN with sampling
         * to keep field logs clean. */
        static uint32_t s_lock_timeout_cnt = 0;
        if ((++s_lock_timeout_cnt % 50) == 1) {
            ESP_LOGW(TAG, "Could not lock ws-client within %" PRIu32 " ms (cnt=%lu)",
                     timeout, (unsigned long)s_lock_timeout_cnt);
        }
        return ESP_FAIL;
    }
